...
export default async function GardenerPage({
  ...
  const iFollow = user.followers.some(
    (f: (typeof user.followers)[number]) =>
      f.followerId === me.id
  );
        ...
        <section className="py-12">
          ...
          {user.gardenPlants.length === 0 ? (
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {user.gardenPlants.map(
                (
                  gp: (typeof user.gardenPlants)[number]
                ) => {
                  const story = gp.plant.stories.find(
                    (
                      s: (typeof gp.plant.stories)[number]
                    ) => s.userId === user.id
                  );
                  ...
                  return story ? (
                    ...
                    <div
                      key={gp.id}
                      className="rounded-lg border border-line bg-fill p-5"
                    >
                      {inner}
                    </div>
                  );
        ...
        {user.listings.length > 0 && (
          <section className="border-t border-line py-12">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {user.listings.map(
                (
                  l: (typeof user.listings)[number]
                ) => (
                  <div
                    key={l.id}
                    className="flex flex-col rounded-lg border border-line bg-fill p-5"
                  >
        ...
        {user.growLogs.length > 0 && (
          <section className="border-t border-line py-12">
            ...
            <div className="mt-6 max-w-[640px]">
              <Timeline
                entries={user.growLogs.map(
                  (
                    gl: (typeof user.growLogs)[number]
                  ) => ({
                    id: gl.id,
                    day: gl.day,
                    note: gl.note,
