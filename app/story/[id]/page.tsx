...
function pickPullQuote(
  ...
  const candidates: string[] = sentences
...
export default async function StoryPage({
  ...
  const pullQuote: string | null =
    pickPullQuote(story.body);

  const quoteAfter: number = Math.min(
    1,
    paragraphs.length - 2
  );
    ...
    <article>
      ...
      <div className="bg-ink">
        <div className="mx-auto max-w-[1080px] px-5 py-14">
          ...
          >
            <Avatar
              seed={story.user.avatarSeed}
              src={story.user.avatarData}
        ...
        <div className="mt-12 grid gap-4 border-t border-line pt-12 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-lg border border-line bg-fill p-5">
            ...
            <ul className="mt-3 space-y-2 text-[15px] text-graphite">
              ...
              <li>
                About{" "}
                {story.plant.daysToHarvest >= 365
                  ? `${Math.round(
                      story.plant.daysToHarvest /
                        365
                    )} year${
                      story.plant.daysToHarvest >=
                      730
          ...
          <div className="rounded-lg border border-line bg-fill p-5">
            ...
            <p className="mt-2 text-[15px] leading-6 text-graphite">
              {story.plant.commonName} is native
              to the{" "}
              {story.plant.origin
                .charAt(0)
                .toLowerCase() +
                story.plant.origin.slice(1)}
              . This one made its way to{" "}
              {story.user.suburb} with{" "}
              {story.user.name.split(" ")[0]}
              &apos;s family, and the story above is
              theirs, told in their own words.
            </p>
          </div>
          ...
          >
            <div>
              ...
              <p className="mt-2 text-[15px] leading-6 text-graphite">
                See every neighbour growing{" "}
                {story.plant.commonName.toLowerCase()}{" "}
                on the map.
              </p>
            </div>
