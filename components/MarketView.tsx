...
const MarketMap = dynamic(
  () => import("@/components/MarketMap"),
  {
    ssr: false,
    loading: () => (
      <div
        className="h-full w-full animate-pulse bg-fill"
...
const sortOptions = [
  {
    value: "distance",
    label: "Sort: nearest first",
  },
  {
    value: "type-culture",
    label: "Sort: type, then culture",
  },
  {
    value: "culture",
    label: "Sort: culture",
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-[640px] text-[15px] text-graphite">
            Vegetables, herbs, fruit and tools from
            gardens near you. The map shows where
      ...
      {filtered.length === 0 ? (
        <div className="mt-10 rounded-lg border border-line bg-fill px-6 py-14 text-center">
          ...
          <p className="mx-auto mt-1 max-w-[400px] text-[15px] text-graphite">
            Try a different search, or clear a
            filter to browse everything nearby.
          </p>
        </div>
      ) : (
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((l) => (
            <div
              ...
              className={`flex cursor-pointer flex-col rounded-lg border bg-fill p-5 transition-colors duration-150 ${
                selectedId === l.id
                  ? "border-green"
                  : "border-line"
              } ${
