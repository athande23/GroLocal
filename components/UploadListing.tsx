...
const categories = ["vegetable", "herb", "fruit", "legume", "flower", "tool", "other"];

export default function UploadListing({ defaultAddress }: { defaultAddress: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setOpen(false);
      router.push(`/market?q=${encodeURIComponent(title.trim())}`);
      router.refresh();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Share something">
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            <span className="text-[13px] font-medium text-graphite">Photo</span>
            <div className="mt-1 flex items-center gap-3">
              {imageData ? (
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-lg border border-dashed border-line bg-fill">
                  <ImageUp size={18} strokeWidth={1.5} className="text-graphite" />
                </div>
              )}
              <button
                type="button"
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-[13px] font-medium text-graphite">Type</span>
              <select
                value={category}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              <span className="text-[13px] font-medium text-graphite">Quantity</span>
              <input
                value={quantity}
          ...
          <Button type="submit" disabled={busy} className="w-full justify-center">
            {busy ? "Sharing…" : "Share it on the market"}
          </Button>
        </form>
      </Modal>
    </>
